...
<script>
    (function() {
        ...
        formEl.addEventListener('submit', function(e) {
            if (idEl.value === '' &&
                (skEl.value || thEl.value || ssEl.value)) {
                e.preventDefault();
                alert('Kombinasi konteks tidak ditemukan.');
            }
        });
        ...
        resetBtn.addEventListener('click', function() {
            ...
            fetch("<?= site_url('penetapan-konteks/set-active-konteks') ?>", {
                method: "POST",
                headers: {
                    "Content-Type": "application/x-www-form-urlencoded",
                    "X-CSRF-TOKEN": "<?= csrf_hash() ?>"
                },
                body: "id_konteks=&redirect=<?= current_url() ?>"
            }).then(() => {
