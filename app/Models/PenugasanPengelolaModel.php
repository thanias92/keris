...
class PenugasanPengelolaModel extends Model
{
    ...
    // -------------------------------------------------------
    // Ambil pengelola (ketua tim) berdasarkan tim kerja & tahun
    // -------------------------------------------------------
    public function getKetuaTim(int $tim_kerja_id, int $tahun): ?array
    {
    ...
    // -------------------------------------------------------
    public function getPengelolaByTimKerja(int $tim_kerja_id, int $tahun): array
    {
        return $this->db->table('penugasan_pengelola pp')
            ->select('pr.*, pp.tahun, pp.is_ketua_tim')
            ->join('pengelola_risiko pr', 'pr.id = pp.pengelola_id')
    ...
    // -------------------------------------------------------
    public function getKetuaTimWithFallback(int $tim_kerja_id, int $tahun): ?array
    {
        // Coba cari tahun yang diminta dulu
        $result = $this->getKetuaTim($tim_kerja_id, $tahun);
    ...
    // -------------------------------------------------------
    // Cek apakah sudah ada penugasan untuk tim kerja & tahun
    // -------------------------------------------------------
    public function sudahAdaPenugasan(int $tim_kerja_id, int $tahun): bool
    {
        return $this->where('tim_kerja_id', $tim_kerja_id)
            ->where('tahun', $tahun)
            ->countAllResults() > 0;
    }
}
